...
describe('Generator-page+frame-command: create frame with name "banana" on page "apple"', () => {
    config();
    ...
    describe('#prepare()', () => {
        ...
        it(`rucken page+frame --page-name apple --frame-name banana --root ./test/fixture`, (done) => {
            const file = path.resolve(`${dirRoot}/dist/bin/app.js`);
            const commandString = 'node . page+frame --page-name apple --frame-name banana --root ./test/fixture ' + (debug ? ' --verbose' : '');
